...
class World(Object):
    ...
    def __init__(self, name, export_config, local_transform=Matrix()):
...
class Poly(Object):
    ...
    def _parse(self, ac_mats, str_pre):
        if self.bl_obj:
            TRACE(
                "{0}  ~ ({1}) {2}".format(
                    str_pre, self.bl_obj.type, self.bl_obj.data.name
                )
            )
    ...
    def _parseMesh(self, ac_mats):
        ...
        if useCopy:
            ...
            mesh = self.bl_obj_copy.to_mesh(
                preserve_all_data_layers=True, depsgraph=depsgraph
            )  # 2.79 was: self.export_config.context.scene, True, 'PREVIEW'
        ...
        if useCopy:
            ...
            bpy.data.objects.remove(
                self.bl_obj_copy, do_unlink=True, do_id_user=True, do_ui_user=True
            )
            bpy.data.collections.remove(
                col, do_unlink=True, do_id_user=True, do_ui_user=True
            )
            bpy.data.meshes.remove(
                mesh_copy, do_unlink=True, do_id_user=True, do_ui_user=True
            )
            ...
            if not len(self.tex_name):
                if bl_mat.node_tree:
                    ...
                    if doSearch:
                        ...
                        textures = []
                        textures.extend(
                            [x for x in bl_mat.node_tree.nodes if x.type == "TEX_IMAGE"]
                        )  # 2.79 was: #for tex_slot in bl_mat.node_tree.texture_slots:
                        print(textures)
                        for bl_tex in textures:
                            ...
                            if bl_im is None:
                                print(
                                    "Texture has no image data (skipping): "
                                    "Tex name="
                                    + bl_tex.name
                                    + " Mat name="
                                    + bl_mat.name
            ...
            if not os.path.exists(export_tex):
                if image.packed_file:
                    ...
                    if not os.path.exists(abs_path):
                        TRACE(
                            "Warning: Texture doesn't exists: {0}".format(
                                image.filepath
                            )
                        )
                    else:
    ...
    def _parseFaces(self, mesh):
        ...
        is_flipped = (
            self.bl_obj.scale[0] * self.bl_obj.scale[1] * self.bl_obj.scale[2] < 0
        )
        ...
        for face_idx in range(len(mesh.polygons)):
            ...
            surf = self.Surface(
                ...
                is_flipped,  # show_double_sided is no longer supported in Blender 2.80
                uv_coords,
                0,
            )
        ...
        def write(self, ac_file):
            ...
            if self.uv_coords:
                for n in r:
                    ...
                    u = "{0:.6f}".format(round(uv_ref[0], 6)).rstrip("0").rstrip(".")
...
class Material:
    ...
    def __init__(self, name="DefaultWhite", bl_mat=None, export_config=None):
        ...
        if bl_mat:
            ...
            try:
                ...
                curr_shader = (
                    links[0].from_node
                )  # This works, but not 100% that the first link is always Surface
                ...
                if curr_shader.type == "BSDF_PRINCIPLED":
                    ...
                    self.emis = [
                        emis.default_value * base.default_value[0],
                        emis.default_value * base.default_value[1],
                        emis.default_value * base.default_value[2],
                    ]
                    self.rgb = [0, 0, 0]
                    self.amb = [0, 0, 0]
                    self.spec = [0, 0, 0]
                    ...
                    print(
                        "Material "
                        + bl_mat.name
                        + " is not Principled BSDF or Specular using nodes, Emission not exported"
                    )
            except:
                ...
                print(
                    "Material "
                    + bl_mat.name
                    + " is not using nodes, Emission not exported"
                )
            ...
            blRange = blMax - blMin
            self.shi = int(
                round((((float(rough) - blMin) * acRange) / blRange) + acMin, 0)
            )
    ...
    def same_as(self, rhs):
        ...
        if self.merge:
            return (
                self._feq(self.rgb[0], rhs.rgb[0])
                and self._feq(self.rgb[1], rhs.rgb[1])
                and self._feq(self.rgb[2], rhs.rgb[2])
                and self._feq(self.amb[0], rhs.amb[0])
                and self._feq(self.amb[1], rhs.amb[1])
                and self._feq(self.amb[2], rhs.amb[2])
                and self._feq(self.emis[0], rhs.emis[0])
                and self._feq(self.emis[1], rhs.emis[1])
                and self._feq(self.emis[2], rhs.emis[2])
                and self._feq(self.spec[0], rhs.spec[0])
                and self._feq(self.spec[1], rhs.spec[1])
                and self._feq(self.spec[2], rhs.spec[2])
                and self._feq(self.shi, rhs.shi)
                and self._feq(self.trans, rhs.trans)
            )
